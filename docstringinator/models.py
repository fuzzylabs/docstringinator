...
class FormatConfig(BaseModel):
    ...
    style: DocstringFormat = Field(default=DocstringFormat.GOOGLE, description="Docstring style")
    include_examples: bool = Field(default=True, description="Include examples in docstrings")
    include_type_hints: bool = Field(default=True, description="Include type hints in docstrings")
...
class ProcessingConfig(BaseModel):
    ...
    backup_files: bool = Field(default=True, description="Create backup files before modifying")
    max_file_size: int = Field(default=1_000_000, description="Maximum file size in bytes")
...
class OutputConfig(BaseModel):
    ...
    output_format: str = Field(default="text", description="Output format (text, json, yaml)")
...
class DocstringInfo(BaseModel):
    ...
    class_name: Optional[str] = Field(default=None, description="Name of the class if method")
    module_name: str = Field(description="Name of the module")
    signature: str = Field(description="Function signature")
    existing_docstring: Optional[str] = Field(default=None, description="Existing docstring if any")
    ...
    has_docstring: bool = Field(description="Whether the function already has a docstring")
...
class ProcessingResult(BaseModel):
    ...
    changes: List[Change] = Field(default_factory=list, description="Changes made to the file")
...
class BatchResult(BaseModel):
    ...
    results: List[ProcessingResult] = Field(default_factory=list, description="Individual file results")
